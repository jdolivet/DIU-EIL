# Outil permettant da visualiser la représentation des arbres binaires
# Ce module utilise le module binaryTree de Éric Wegrzynowski
# Il utilise les bibliothèques Graphviz
    ...
    def to_dot(self, background_color=WHITE):
        ...
        def aux(arbre, prefix=''):
            if arbre.is_empty():
                ...
                label_lien, couleur_lien = ('', BLACK) if not s_a_g.is_empty() else ('', background_color)
                descr = (descr +
                         aux(s_a_g, prefix+'0') +
                         LIEN.format(prefix, prefix+'0', couleur_lien, label_lien))
                s_a_d = arbre.get_right_subtree()
                label_lien, couleur_lien = ('', BLACK) if not s_a_d.is_empty() else ('', background_color)
                descr = (descr +
                         aux(s_a_d, prefix+'1') +
...
def copie_binary_tree(noeud):
    """ retourne la copie d'un noeud en binaryTree"""
    if noeud.est_vide():
        return BinaryTree()
    else:
        return BinaryTree(noeud.etiquette(),
                     copie_binary_tree(noeud.gauche()),
                     copie_binary_tree(noeud.droit()))

def representation(tree):
    binaryTree = copie_binary_tree(tree)
    display(graphviz.Source(binaryTree.to_dot()))
